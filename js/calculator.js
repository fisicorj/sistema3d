...
function switchCalcTab(name, btn) {
    ...
    document.getElementById('calcTab-' + name).classList.add('active');
...
function calculatePrice() {
    ...
    const shippingCost = calculateShipping(shippingWeight);
    const shippingInfo = getShippingInfo(shippingWeight);
...
function updatePriceCalculation() {
    ...
    if (linesEl) {
        ...
        const shipLabel = p.shippingInfo ? (p.shippingInfo.source === 'melhor_envio' ? `Frete Melhor Envio (${p.shippingInfo.region}${p.shippingInfo.deliveryDays ? ' • ' + p.shippingInfo.deliveryDays + ' dias' : ''})` : `Frete por CEP (${p.shippingInfo.uf}/${p.shippingInfo.region}${p.shippingInfo.deliveryDays ? ' • ' + p.shippingInfo.deliveryDays + ' dias' : ''})`) : 'Frete estimado';
        ...
        linesEl.innerHTML = rows.join('') + (filamentLine ? `<div style="font-size:0.78em;margin-top:8px;color:rgba(255,255,255,0.85);">${filamentLine}</div>` : '') + badgeHtml;
    }
...
function resultRow(label, value, cls = '') {
    let color = cls.includes('danger') ? ' style="color:#f87171;"' : '';
    if (cls.includes('muted')) color = ' style="color:rgba(255,255,255,0.65);"';
    const strong = cls.includes('strong') ? 'font-size:1.05em;font-weight:800;' : '';
    const profit = cls.includes('profit') ? ' profit' : '';
    return `<div class="result-line" style="${strong}"><span class="result-line-label">${label}</span><span class="result-line-value${profit}"${color}>${value}</span></div>`;
}
