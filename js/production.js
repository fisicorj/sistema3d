...
function loadProductionBoard(){
  ...
  el.innerHTML=PRODUCTION_COLUMNS.map(([status,label,icon])=>{
    const cards=rows.filter(r=>r[7]===status).map(r=>productionCard(r,status)).join('')||'<div class="kanban-empty">Nenhum pedido</div>';
    return `<section class="kanban-column"><header><span>${icon} ${label}</span><b>${count(status)}</b></header><div class="kanban-stack">${cards}</div></section>`;
  }).join('');
}
