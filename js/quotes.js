...
function loadQuotes(page = quotesCurrentPage) {
    ...
    if (!rows.length || !rows[0].values.length) {
        tbody.innerHTML = '<tr><td colspan="7" class="text-center text-body-secondary py-5"><i class="bi bi-file-earmark-text d-block fs-3 mb-2"></i>Nenhum orçamento registrado ainda.</td></tr>';
    ...
    const quoteValues = rows[0].values;
    const setText = (id, value) => { const node = document.getElementById(id); if (node) node.textContent = value; };
    const waiting = quoteValues.filter(r => String(r[7] || '').toLowerCase() === 'aguardando').length;
    const accepted = quoteValues.filter(r => String(r[7] || '').toLowerCase() === 'aceito').length;
    const openValue = quoteValues.filter(r => String(r[7] || '').toLowerCase() === 'aguardando').reduce((sum,r)=>sum+Number(r[5]||0)+Number(r[6]||0),0);
    setText('quotesWaitingCount', waiting);
    setText('quotesAcceptedCount', accepted);
    setText('quotesOpenValue', money(openValue));
    setText('quotesConversionRate', quoteValues.length ? `${Math.round(quoteValues.filter(r => r[10]).length / quoteValues.length * 100)}%` : '0%');
    ...
    tbody.innerHTML = rows[0].values.map(([id, clientName, desc, qty, unitPrice, total, shipping, status, createdAt, validityDate, orderId]) => {
        ...
        const convertBtn = canConvert
            ? `<button class="btn btn-primary btn-sm" onclick="convertQuoteToOrder(${id})">📦 Criar Pedido</button>`
        ...
        return `<tr>
            ...
            <td class="text-end text-nowrap">
                <div class="d-inline-flex flex-nowrap align-items-center gap-1">
                    ${st === 'aguardando' ? `<div class="s3d-actions d-inline-flex flex-row flex-nowrap align-items-center gap-1" role="group"><button class="btn btn-outline-success" title="Aceitar orçamento" onclick="setQuoteStatus(${id},'aceito')"><i class="bi bi-check-lg"></i></button>
                    <button class="btn btn-outline-danger" title="Recusar orçamento" onclick="setQuoteStatus(${id},'recusado')"><i class="bi bi-x-lg"></i></button></div>` : ''}
                    ${convertBtn}
                    <div class="s3d-actions d-inline-flex flex-row flex-nowrap align-items-center gap-1" role="group">
                        <button class="btn btn-outline-secondary" title="Anexos" onclick="openAttachments('quotes',${id},'Orçamento #${id}')"><i class="bi bi-paperclip"></i></button>
                        <button class="btn btn-outline-danger" title="Excluir orçamento" onclick="deleteQuote(${id})"><i class="bi bi-trash"></i></button>
                    </div>
                </div>
            </td>
        </tr>`;
    ...
    if (pager) {
        ...
        pager.innerHTML = `<div class="d-flex align-items-center justify-content-between gap-3 flex-wrap">
            <span class="small text-body-secondary">Exibindo ${first}–${last} de ${totalRows}</span>
            <div class="d-flex align-items-center gap-2">
                <button class="btn btn-outline-secondary btn-sm" onclick="loadQuotes(${quotesCurrentPage - 1})" ${quotesCurrentPage <= 1 ? 'disabled' : ''}>← Anterior</button>
                <span class="small text-body-secondary">Página ${quotesCurrentPage} de ${totalPages}</span>
                <button class="btn btn-outline-secondary btn-sm" onclick="loadQuotes(${quotesCurrentPage + 1})" ${quotesCurrentPage >= totalPages ? 'disabled' : ''}>Próxima →</button>
            </div>
        </div>`;
    }
}
...
function convertQuoteToOrder(quoteId) {
    ...
    document.getElementById('modalBody').innerHTML = `
        ...
        <div style="display:flex;gap:8px;margin-top:16px;">
            <button class="btn btn-primary" onclick="_doConvertQuote(${quoteId}, ${clientId || 'null'}, ${qty}, ${unitPrice}, ${totalPrice}, ${shippingCost})">✅ Criar Pedido</button>
...
function _doConvertQuote(quoteId, clientId, qty, unitPrice, totalPrice, shippingCost) {
    ...
    // Usa loaders originais (não-REST-wrapped) para renderizar o estado local imediatamente,
    // antes do applyDiff sincronizar com o servidor.
    (window.RestModules?.originals?.loadQuotes || loadQuotes)();
    const _origLoadOrders = window.RestModules?.originals?.loadOrders;
