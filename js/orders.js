...
function loadOrders(resetPage = false) {
    ...
    const baseQuery = `SELECT o.id, o.work_type, o.material_name, o.weight, o.print_time,
                        ...
                        o.printing_started_at, o.paid_amount
                 FROM orders o
    ...
    if (result.length > 0 && result[0].values.length > 0) {
        result[0].values.forEach(o => {
            const [id, workType, materialName, weight, printTime,
                   ...
                   printingStartedAt, paidAmount] = o;
            ...
            const bambuEl = isPrinting
                ? `<div data-bambu-widget style="margin-top:2px;"></div>` : '';

            if (showDeleted) {
                html += `
                    <div class="order-item" style="opacity:0.75;">
                        ...
                        <div class="order-actions">
                            <button class="btn-primary btn-sm" onclick="restoreOrder(${id})">↩️ Restaurar</button>
                            <button class="btn-danger btn-sm"  onclick="hardDeleteOrder(${id})">❌ Apagar</button>
                        </div>
                    </div>`;
            } else {
                html += `
        ...
        html += '<p style="text-align:center; color:#888; padding:20px;">Nenhum pedido encontrado.</p>';
    }
    ...
    if (total > ORDERS_PER_PAGE) {
        ...
        html += `<div style="display:flex;align-items:center;justify-content:center;gap:12px;padding:12px 0;font-size:0.9em;">
            <button class="btn-primary btn-sm" onclick="_ordersPage--;loadOrders()" ${_ordersPage === 0 ? 'disabled' : ''}>← Anterior</button>
            <span>${from}–${to} de ${total}</span>
            <button class="btn-primary btn-sm" onclick="_ordersPage++;loadOrders()" ${to >= total ? 'disabled' : ''}>Próxima →</button>
        </div>`;
    }
...
function showOrderNotes(orderId) {
    ...
    document.getElementById('modalBody').innerHTML = `
        ...
        <button class="btn-primary" onclick="saveOrderNote(${orderId})">💾 Salvar nota</button>`;
    openModal();
}
...
function registerPayment(orderId, totalPrice, paidSoFar) {
    ...
    document.getElementById('modalBody').innerHTML = `
        ...
        <div style="display:flex;gap:8px;margin-top:10px;">
            <button class="btn-primary" onclick="savePayment(${orderId}, ${paidSoFar})">💾 Registrar</button>
            <button class="btn-success" onclick="savePayment(${orderId}, 0, true)">✅ Quitado total</button>
        </div>`;
    openModal();
}
...
<div class="label">
  ...
  <div class="row"><span class="lbl">Tipo</span><span>${workTypeNames[wt]||wt}</span></div>
  ...
  <div class="row"><span class="lbl">Status</span><span>${statusNames[status]||status}</span></div>
    ...
    Object.values(byPrinter).forEach(({ name: pName, items, totalH }) => {
        ...
        items.forEach(([id, wt, mat, weight, pt, qty, status, date, cName]) => {
            ...
            html += `<div style="display:flex;justify-content:space-between;align-items:center;
                ...
                <div>
                    <strong>#${id}</strong> · ${workTypeNames[wt]||wt} · ${h(mat||'—')} · ${weight}g × ${qty}
                ...
                <div style="display:flex;gap:6px;align-items:center;">
                    ...
                    <span class="status-badge status-${status}">${statusNames[status]||status}</span>
                </div>
            </div>`;
        });
        html += '</div>';
