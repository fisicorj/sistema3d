    ...
    if (result.length > 0 && result[0].values.length > 0) {
        result[0].values.forEach(([id, name, color, spoolW, cost, stock, minAlert]) => {
            ...
            rows += `<tr>
                ...
                <td>${minAlert}g</td>
                <td>
                    <button class="btn-warning btn-sm" onclick="addStock(${id})">➕ Estoque</button>
                    <button class="btn-info    btn-sm" onclick="showStockHistory(${id}, '${h(name)} ${h(color)}')">📋</button>
                    <button class="btn-info    btn-sm" onclick="showEditMaterialModal(${id})">✏️</button>
                    <button class="btn-danger  btn-sm" onclick="deleteMaterial(${id})">🗑️</button>
                </td>
            </tr>`;
