...
// ── Bootstrap da tela ───────────────────────────────────────────────────────
function loadInsights(force=false){if(!_ensureIdeasTable())return;if(!_insightsInitialized||force){_insightsInitialized=true;buildRadarEditorControls();buildRadarWeightSettings();resetRadarForm();searchMarket(_insightsCurrentQuery)}renderRadarAll()}
function renderRadarAll(){updateRadarKpis();renderRadarOverview();renderRadarPipeline();renderRadarHistory()}
function radarShowTab(target){const trigger=document.querySelector(`[data-bs-target="${target}"]`);if(trigger)bootstrap.Tab.getOrCreateInstance(trigger).show()}
...
function buildRadarEditorControls(){
  ...
  if(s)s.innerHTML=RADAR_SLIDERS.map(([id,label,,value,negative])=>`<div class="radar-slider-row"><label>${label}<b id="${id}Val">${value}</b></label><input class="form-range" id="${id}" type="range" min="0" max="10" value="${value}" oninput="document.getElementById('${id}Val').textContent=this.value;updateOpportunityScore()" style="accent-color:${negative?'#fd7e14':'var(--bs-primary)'}"></div>`).join('');
  const e=document.getElementById('radarEvidenceChecks');
  if(e)e.innerHTML=RADAR_EVIDENCE.map(([id,label])=>`<div class="col-6"><label class="radar-evidence"><input class="form-check-input me-2" id="${id}" type="checkbox" onchange="updateOpportunityScore()">${label}</label></div>`).join('')
}
...
function buildRadarWeightSettings(){
  ...
  el.innerHTML=Object.entries(labels).map(([k,l])=>`<div class="radar-weight-row"><label>${l}</label><input class="form-range" id="radarWeight_${k}" type="range" min="0" max="40" value="${w[k]}" oninput="document.getElementById('radarWeightValue_${k}').textContent=this.value+'%'"><strong id="radarWeightValue_${k}">${w[k]}%</strong></div>`).join('')
}
...
function renderRadarOverview(){
  ...
  if(el)el.innerHTML=top.length?top.map((i,n)=>{
    ...
    return `<div class="radar-top-card">
...
function renderRadarPipeline(){
  ...
  el.innerHTML=columns.map(status=>{
    ...
    return `<section class="radar-column" data-status="${status}" ondragover="radarDragOver(event)" ondragleave="this.classList.remove('drag-over')" ondrop="radarDrop(event,'${status}')">
      <div class="radar-column-header"><span class="radar-column-title"><i class="radar-column-dot" style="--status-color:${st.color}"></i>${st.label}</span><span class="radar-column-count">${items.length}</span></div>
...
function radarKanbanCard(i){
  ...
  return `<article class="radar-kanban-card" draggable="true" ondragstart="radarDragStart(event,${i.id})" ondragend="radarDragEnd(event)">
