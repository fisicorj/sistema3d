...
window.RelationalSync = (() => {
    ...
    function insertMirrorRow(table, item) {
        ...
        // Normaliza campos obrigatórios antes de popular o espelho sql.js.
        // APIs e bancos externos podem devolver null mesmo quando o modelo possui default.
        const normalized = { ...item };
        if (table === 'printers') {
            normalized.name = String(normalized.name || 'Impressora');
            for (const field of ['value','lifetime_hours','wattage','speed_gph','hours_used']) {
                const value = Number(normalized[field]);
                normalized[field] = Number.isFinite(value) && value >= 0 ? value : 0;
            }
        }
        const cols = allowed.filter(c => Object.prototype.hasOwnProperty.call(normalized, c));
        if (!cols.length) return;
        const placeholders = cols.map(() => '?').join(',');
        db.run(`INSERT INTO ${table} (${cols.join(',')}) VALUES (${placeholders})`, cols.map(c => normalized[c]));
    }
