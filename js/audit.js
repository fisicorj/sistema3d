const AUDIT_RESTORABLE_TABLES = new Set(['clients','products','orders','quotes']);
function safeAuditTable(value){ const v=String(value||''); return AUDIT_RESTORABLE_TABLES.has(v)?v:''; }
...
function loadTrash() {
    ...
    body.innerHTML=items.length?items.slice(0,100).map(x=>`<tr><td>${h(x.table)}</td><td>${h(x.name||x.id)}</td><td>${new Date(x.dt).toLocaleString('pt-BR')}</td><td><button class="btn-secondary btn-sm" onclick="restoreDeletedRecord('${safeAuditTable(x.table)}',${Number(x.id)})">Restaurar</button></td></tr>`).join(''):'<tr><td colspan="4" style="text-align:center;color:var(--text-muted)">A lixeira está vazia.</td></tr>';
}
