    ...
    rows[0].values.forEach(([id, cat, desc, amount, recur, date, recurrenceParentId]) => {
        ...
        tableRows += `<tr>
            ...
            <td class="text-end text-nowrap">
                <div class="s3d-actions d-inline-flex flex-row flex-nowrap align-items-center gap-1" role="group" aria-label="Ações da despesa">
                    <button class="btn btn-outline-primary" title="Editar despesa" onclick="showExpenseModal(${id})"><i class="bi bi-pencil"></i></button>
                    <button class="btn btn-outline-danger" title="Excluir despesa" onclick="deleteExpense(${id})"><i class="bi bi-trash"></i></button>
                </div>
            </td>
        </tr>`;
    });
